...
public class ForgeRegistryHelper implements IRegistryHelper {
    ...
    public static final DeferredRegister<SoundEvent> SOUNDS = DeferredRegister.create(ForgeRegistries.SOUND_EVENTS, ExcessiveBuilding.MOD_ID);
    ...
    public Supplier<SoundEvent> registerSound(String id) {
        return SOUNDS.register(id, () -> SoundEvent.createVariableRangeEvent(new ResourceLocation(ExcessiveBuilding.MOD_ID, id)));
    }
    ...
    private static class RegistryMap {
        ...
        private <T> RegistryObject<T> register(Registry<? super T> registry, String name, Supplier<T> entry) {
            DeferredRegister<T> reg = (DeferredRegister<T>)registries.computeIfAbsent(registry.key().location(), (key) -> {
                ...
                DeferredRegister<T> defReg = DeferredRegister.create(forgeReg, ExcessiveBuilding.MOD_ID);
