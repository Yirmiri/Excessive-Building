...
public class EBUtils {
    public static void hammerUsed(Level world, BlockPos pos, BlockState state, Player player) {
        player.playSound(SoundEvents.UI_STONECUTTER_TAKE_RESULT, 0.5F, 1.0F);
        world.addDestroyBlockEffect(pos, state);
        player.awardStat(EBStats.BLOCKS_HAMMERED);
    }

    public static void wrenchUsed(Player player) {
        player.playSound(SoundEvents.SPYGLASS_USE, 0.5F, 1.0F);
        player.awardStat(EBStats.BLOCKS_WRENCHED);
    }
