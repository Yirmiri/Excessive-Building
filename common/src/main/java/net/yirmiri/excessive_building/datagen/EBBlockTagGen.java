...
public class EBBlockTagGen extends FabricTagProvider.BlockTagProvider {
    ...
    public void appendCannotBeWrenched() {
        tag(EBTags.Blocks.CANNOT_BE_WRENCHED)

        ;
    }

    public void appendCarvedPumpkins() {
        tag(EBTags.Blocks.CARVED_PUMPKINS)
    ...
    public void appendHammerMineable() {
        tag(EBTags.Blocks.HAMMER_MINEABLE)
                ...
                .add(EBBlocks.AMETHYST_LAMP)
        ;

        for (DyeColor colors : DyeColor.values()) {
            tag(EBTags.Blocks.HAMMER_MINEABLE).add(EBBlocks.getDyedAlmentraStatues(colors.getId()));
            tag(EBTags.Blocks.HAMMER_MINEABLE).add(EBBlocks.getDyedTerracottaPots(colors.getId()));
        }
    }

    public void appendTerracottaPots() {
        tag(EBTags.Blocks.TERRACOTTA_POTS)
                .add(EBBlocks.TERRACOTTA_POT)
        ;

        for (DyeColor colors : DyeColor.values()) {
            tag(EBTags.Blocks.TERRACOTTA_POTS).add(EBBlocks.getDyedTerracottaPots(colors.getId()));
        }
    }
    ...
    public void appendDecoratives() {
        ...
        for (DyeColor colors : DyeColor.values()) {
            tag(EBTags.Blocks.DECORATIVES).add(EBBlocks.getDyedAlmentraStatues(colors.getId()));
        }
    }
